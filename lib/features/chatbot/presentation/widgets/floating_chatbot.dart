...
class _FloatingChatbotState extends ConsumerState<FloatingChatbot> {
  ...
  Widget _buildChatPanel(ChatState chatState, ThemeData theme, Color primary) {
    ...
    final panelHeight = (size.height - 150 - (bottomInset > 0 ? bottomInset : 0)).clamp(0.0, 560.0);
        ...
        child: BackdropFilter(
          ...
          child: Column(
            children: [
              ...
              // Quick Actions
              if (chatState.messages.isEmpty) _buildQuickActions(isDark, primary),
    ...
    ).animate().fadeIn(duration: 400.ms).moveY(begin: 20, end: 0, curve: Curves.easeOutBack),
    );
  }
  ...
  Widget _buildPulseDot(Color primary) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        shape: BoxShape.circle,
        boxShadow: [
          BoxShadow(color: primary.withValues(alpha: 0.4), blurRadius: 8, spreadRadius: 2),
        ],
      ),
  ...
  Widget _buildQuickActions(bool isDark, Color primary) {
    final actions = ['Check Loan Stats', 'Savings Growth', 'System Health', 'Security Audit'];
    return Padding(
      ...
      child: Wrap(
        ...
        children: actions.map((action) => GestureDetector(
          onTap: () => ref.read(chatProvider.notifier).sendMessage(action),
          child: Container(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.auto_awesome_rounded, color: primary.withValues(alpha: 0.3), size: 40),
            ),
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: isUser ? null : (isDark ? Colors.white.withValues(alpha: 0.06) : Colors.black.withValues(alpha: 0.04)),
          ...
          children: [
            if (cleanText.isNotEmpty)
              Text(
                cleanText,
                style: TextStyle(
                  color: isUser ? Colors.white : (isDark ? Colors.white.withValues(alpha: 0.9) : Colors.black),
                  fontSize: 13,
            ...
            children: [
              Row(
                children: [
                  ...
                  Text('Live Portfolio', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 11, color: primary)),
                ],
              ),
              ...
              summaryAsync.when(
                ...
                error: (e, _) => Text('Error loading metrics', style: TextStyle(color: Colors.red, fontSize: 10)),
              ),
            ],
          ),
        );
  ...
  Widget _buildLiveMetrics(dynamic data, bool isDark, Color primary) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Portfolio at Risk (PAR)', style: TextStyle(fontSize: 10, color: isDark ? Colors.white70 : Colors.black54)),
        const SizedBox(height: 4),
        ClipRRect(
        ...
        Row(
          ...
          children: [
            _buildMetricStat('Active', '${data.activeLoans}', Colors.green, isDark),
            _buildMetricStat('Default', '${data.defaultLoans}', Colors.red, isDark),
        ...
        child: Row(
          ...
          children: List.generate(3, (i) => 
            Container(
              ...
              decoration: BoxDecoration(color: primary.withValues(alpha: 0.3), shape: BoxShape.circle),
            ).animate(onPlay: (c) => c.repeat()).scale(delay: (i * 200).ms, duration: 800.ms, begin: const Offset(1,1), end: const Offset(1.6,1.6))
          ),
        ),
      ),
    );
  ...
  Widget _buildVoiceButton(ChatState chatState, Color primary) {
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(
          color: chatState.isListening ? Colors.red.withValues(alpha: 0.15) : primary.withValues(alpha: 0.1),
          shape: BoxShape.circle,
        ),
        child: Icon(
      ...
      ).animate(target: chatState.isListening ? 1 : 0).shimmer(color: Colors.white).scale(begin: const Offset(1,1), end: const Offset(1.2,1.2), duration: 500.ms, curve: Curves.elasticOut),
    );
  }
